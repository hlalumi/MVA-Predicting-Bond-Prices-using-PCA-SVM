...
This study used PCA to identify the main underlying factors that are responsible for driving South African bond prices and hence interest movements. PCA has several important implications including dimension reduction, prevention of model overfitting, data compression and model accuracy improvement. Previous studies noted duration as being the variable which plays a major influence in bond price movements in volatile markets . However in this study because the duration variable mostly had zeros , this variable was excluded. From the PCA analysis convexity, interest yield and total return index are the variables that explain most of the variation in the bond price data set. Additionally we performed SVM for regression to predict future bond . The SVM does a fairly job at accurately predicting future prices as seen from the RMSE and R-squared statistics.
